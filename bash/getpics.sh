...
#i don't have that files i am going to download it.

test -f ~/Pictures/pics.zip || wget -q -O ~/Pictures/pics.tgz http://zonzorp.net/pics.tgz



#unpacking the files and also deleting them



test -f ~/Pictures/pics.tgz && (tar zxvf ~/Pictures/pics.tgz -C ~/Pictures | awk '{$7}') && rm ~/Pictures/pics.tgz
...
Found $(find ~/Pictures -type f|wc -l) files in  Pictures directory.

The Pictures directory use $(du -sh ~/Pictures|awk '{print $1}') space on disk.
